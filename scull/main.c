...
int scull_read_procmem(struct seq_file *s, void *v){
	int i, j;
	int limit = s->size - 80; /* Dont' print more than this */
...
static void scull_create_proc(void){
	proc_create_data("scullmem", 0 /* default mode */,
			 NULL /* parent dir */,
			 proc_ops_wrapper(&scullmem_proc_ops,scullmem_pops),
			 NULL /* client data */);
	proc_create("scullseq", 0, NULL,
		    proc_ops_wrapper(&scullseq_proc_ops,scullseq_pops));
}
...
struct scull_qset *scull_follow(struct scull_dev *dev, int n){
	...
	/* Allocate first qset explicityly if need be */
	if(!qs){
...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg){
	...
	int retval = 0;
	/*
	 * extract the type and number bitfields, and dont' decode
 ...
 */
/*
 * The cleanup funciton is used to handle initialization failures as well.
...
void scull_cleanup_module(void){
	...
	/* and call the cleanup function for friend devices */
	scull_p_cleanup();
...
static void scull_setup_cdev(struct scull_dev* dev, int index){
	...
	/* Fail gracefully if need to */
	if(err)
		printk(KERN_NOTICE "Erro %d adding scull%d", err, index);
}
int scull_init_module(void){
	...
	/*
	 * allocate the devices -- we can't have them stable, as the number
